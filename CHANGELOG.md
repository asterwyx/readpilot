...
- 更新新版 PNG 图标（16/32/48/128/512px），manifest 增加 `"512"` 图标条目
- 可配置解释输出语言（中文 / English / 跟随浏览器）

### 修复

- 修复 `contextTokenBudget` 在 getConfig 存储键中的缺失
- 解释输出语言固定为选中文本语言
...
- 补充四尺寸扩展图标（16/32/48/128px），manifest 增加 `"32"` 图标条目
- 修复右键点击不弹出解释界面、配置界面无法配置超时的问题
