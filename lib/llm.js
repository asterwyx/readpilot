...
// 区分 abort 来源：外部 signal 已 abort → 用户取消；否则 → 超时
function classifyAbort(signal) {
  return signal?.aborted
    ? { type: "abort", message: "用户取消请求" }
    : { type: "timeout", message: "请求超时，请重试" };
}
...
export function formatError(err) {
  if (!err) return "未知错误";
  if (err.type === "abort") return "请求已取消";
