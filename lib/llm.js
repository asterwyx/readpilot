...
// 解释输出语言：zh 中文 / en 英文 / browser 浏览器默认

// 解析 browser 选项为实际语言：navigator.language 的 zh* → 中文、en* → 英文，其余回退中文
function resolveBrowserLanguage() {
  try {
    const lang = (typeof navigator !== "undefined" && navigator.language) || "";
    if (lang.toLowerCase().startsWith("zh")) return "zh";
    if (lang.toLowerCase().startsWith("en")) return "en";
  } catch { /* navigator 不可用时走回退 */ }
  return "zh"; // 回退中文（本项目面向中文用户）
}

// 组装 system prompt：强调结合网页上下文解释并补充相关知识，按配置固定输出语言
function buildSystemPrompt(customPrompt, explainLanguage = "browser") {
  if (customPrompt && customPrompt.trim()) {
    return customPrompt.trim();
  }
  const lang = explainLanguage === "en" ? "en" : explainLanguage === "zh" ? "zh" : resolveBrowserLanguage();
  ...
  return [
    ...
    "2. 解释应简洁、准确、有条理。",
    `3. 输出语言必须使用${langLabel}，不得随模型波动。`,
    "4. 若选中文本是专业术语或缩写，请展开说明其所属领域和常见用法。"
  ].join("");
}
...
function buildRequestBody(config, selection, pageContext) {
  return {
    model: config.model,
    messages: [
      { role: "system", content: buildSystemPrompt(config.systemPrompt, config.explainLanguage) },
...
// 创建超时 AbortController：默认 120s，可通过 config.timeout（毫秒）配置
function createTimeoutController(signal, timeoutMs = 120000) {
...
async function callOnce(config, selection, pageContext, signal) {
  ...
  const { ctrl, timer } = createTimeoutController(signal, config.timeout);
...
async function callStream(config, selection, pageContext, signal, onChunk) {
  ...
  try {
    ...
    for await (const chunk of parseSSEStream(reader)) {
      // 收到第一个 chunk 后清除超时：此后仅依赖用户取消或网络中断
